...
slab_t* alloc_new_slab(kmem_cache_t* cachep, int user) {
    void* mem = 0;
    if(user) mem = ubuddy_alloc(cachep->slab_pages);
    else mem = buddy_alloc(cachep->slab_pages);
...
void kmem_cache_free(kmem_cache_t* cachep, void* obj) {
    ...
    if(!s) {
        release(&cachep->lock);
        return; // object does not belong to this cache
    }
...
void* ukmalloc(size_t size) {
    ...
    return ukmem_cache_alloc(generic_caches[idx]);
}
...
void kmem_cache_info(kmem_cache_t* cachep) {
    if (!cachep) {
        printf("kmem_cache_info: Invalid cache pointer.\n");
        return;
    }
    ...
    int num_free_slabs = 0;
    int total_used_objects = 0;

    for (slab_t *s = cachep->slabs_full; s; s = s->next) {
        num_full_slabs++;
        total_used_objects += s->used_count;
    }

    for (slab_t *s = cachep->slabs_partial; s; s = s->next) {
        num_partial_slabs++;
        total_used_objects += s->used_count;
    }

    for (slab_t *s = cachep->slabs_free; s; s = s->next) {
        num_free_slabs++;
    }

    int total_slabs = num_full_slabs + num_partial_slabs + num_free_slabs;
    int total_objects = total_slabs * cachep->obj_per_slab;


    int utilization = 0;
    if (total_objects > 0) {
        utilization = (total_used_objects * 100) / total_objects;
    }

    printf("\n--- Cache Info: %s ---\n", cachep->name);
    printf("Object Size (aligned+meta): %d bytes\n", (int)cachep->obj_size);
    printf("Slab Size:                  %d pages (%d bytes)\n", cachep->slab_pages, (int)cachep->slab_pages * BLOCK_SIZE);
    printf("Objects per Slab:           %d\n", cachep->obj_per_slab);
    printf("Slab counts:                Full: %d, Partial: %d, Free: %d\n", num_full_slabs, num_partial_slabs, num_free_slabs);
    printf("Total Objects:              %d\n", total_objects);
    printf("Active Objects:             %d\n", total_used_objects);
    printf("Utilization:                %d%%\n", utilization);
    printf("Total Memory Footprint:     %d bytes\n", total_slabs * cachep->slab_pages * BLOCK_SIZE);
    printf("-------------------------------\n");
